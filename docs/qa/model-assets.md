...
`SettingsSurface` renders `ModelResourceQaGallery` only when Vite replaces that build variable
with `"1"`. Each panel receives a deterministic `statusOverride`, skips the Tauri status command
and subscription, and therefore cannot start a download or mutate model storage during inspection.
...
The normal release build omits the variable and renders the production IPC-backed panel.
