...
#[test]
fn disabling_a_model_route_does_not_change_the_last_selected_route() {
    let app_data = TempDir::new().expect("temporary application data");
    let database_path = app_data.path().join("piu.sqlite3");
    let preferences = RuntimePreferences::open(&database_path).unwrap();
    ...
    preferences
        .set_model_route_enabled(ResourceScope::Global, &selected, false)
        .unwrap();
    ...
    assert_eq!(
        preferences
            .resource_enabled(
                ...
                &RuntimeResource::model_route(selected.clone())
            )
            .unwrap(),
        Some(false)
    );
    assert_eq!(
        preferences.current_selection().unwrap(),
        Some(selected.selection(Some("high")))
    );
}
